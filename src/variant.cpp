...
namespace {
    ...
    // Co Ngu Hanh
    Variant* nguhanh_variant() {
        Variant* v = variant_base()->init();
        v->maxRank = RANK_7;
        v->maxFile = FILE_G;
        v->reset_pieces();
        v->add_piece(CUSTOM_PIECE_1, 'i', "K"); // Kim
        v->add_piece(CUSTOM_PIECE_2, 'u', "K"); // Thuy
        v->add_piece(CUSTOM_PIECE_3, 'm', "K"); // Moc
        v->add_piece(CUSTOM_PIECE_4, 'h', "K"); // Hoa
        v->add_piece(CUSTOM_PIECE_5, 't', "K"); // Tho
        v->add_piece(KING, 'k');
        v->pieceTypes = NGUHANH_PIECES | KING;
        v->startFen = "iumkht1/7/7/7/7/7/IUMKHT1[] w - - 0 1";
        v->castling = false;
        v->doubleStep = false;
        v->capturesToHand = true;
        v->pieceDrops = false;
        v->seirawanGating = true; // Reuse pockets for birth-only drops.
        v->pocketSize = 5;
        v->promotionPawnTypes[WHITE] = v->promotionPawnTypes[BLACK] = NO_PIECE_SET;
        v->promotionPieceTypes[WHITE] = v->promotionPieceTypes[BLACK] = NO_PIECE_SET;
        v->nMoveRuleTypes[WHITE] = v->nMoveRuleTypes[BLACK] = NO_PIECE_SET;
        v->nMoveRule = 0;
        v->nFoldRule = 0;
        v->stalemateValue = -VALUE_MATE;
        v->nguhanh = true;
        for (PieceType pt = CUSTOM_PIECE_1; pt <= CUSTOM_PIECE_5; ++pt)
        {
            v->pieceValue[MG][pt] = 400;
            v->pieceValue[EG][pt] = 400;
        }
        return v;
    }
