...
void Position::set_check_info(StateInfo* si) const {
  ...
  for (PieceSet ps = piece_types(); ps;)
  {
      ...
      si->checkSquares[pt] = ksq == SQ_NONE ? Bitboard(0)
                           : var->nguhanh && (pt == KING || is_nguhanh_piece(pt)) ? nguhanh_steps_bb(ksq) & board_bb(sideToMove, pt)
...
Bitboard Position::attackers_to(Square s, Bitboard occupied, Color c, Bitboard janggiCannons) const {
  ...
  if (var->nguhanh)
  {
      ...
      {
          PieceType pt = pop_lsb(ps);
          if ((pt == KING || is_nguhanh_piece(pt)) && (board_bb(c, pt) & s))
              b |= nguhanh_steps_bb(s) & pieces(c, pt);
      }
      return b;
  }
...
bool Position::legal(Move m) const {
  ...
  if (capture(m) && var->nguhanh && !nguhanh_piece_can_capture(type_of(moved_piece(m)), type_of(piece_on(to))))
      return false;
...
bool Position::pseudo_legal(const Move m) const {
  ...
  if (capture(m) && var->nguhanh && !nguhanh_piece_can_capture(type_of(pc), type_of(piece_on(to))))
      return false;
...
bool Position::gives_check(Move m) const {
  ...
  if (    is_gating(m)
      && ((var->nguhanh ? attacks_from(sideToMove, gating_type(m), gating_square(m))
...
void Position::do_move(Move m, StateInfo& newSt, bool givesCheck) {
  ...
  const bool nguhanhExtraTurn = nguhanh_extra_turn(m);

#ifndef NO_THREADS
  thisThread->nodes.fetch_add(1, std::memory_order_relaxed);
#endif
  Key k = st->key ^ (nguhanhExtraTurn ? Key(0) : Zobrist::side);
  ...
  if (captured)
  {
      ...
      if (captures_to_hand())
      {
          Piece pieceToHand = var->nguhanh ? captured
  ...
  st->checkersBB = nguhanhExtraTurn ? (count<KING>(us) ? attackers_to(square<KING>(us), them) & pieces(them) : Bitboard(0))
                                    : givesCheck ? attackers_to(square<KING>(them), us) & pieces(us) : Bitboard(0);
  assert(nguhanhExtraTurn ? !st->checkersBB : givesCheck == bool(st->checkersBB));
...
void Position::undo_move(Move m) {
  ...
  else
  {
      ...
      if (st->capturedPiece)
      {
          ...
          if (captures_to_hand())
              remove_from_hand(var->nguhanh ? st->capturedPiece
                             : !drop_loop() && st->capturedpromoted ? (st->unpromotedCapturedPiece ? ~st->unpromotedCapturedPiece
...
Key Position::key_after(Move m) const {
  ...
  Key k = st->key ^ (nguhanh_extra_turn(m) ? Key(0) : Zobrist::side);
  ...
  if (captured)
  {
      ...
      if (captures_to_hand())
      {
          Piece removeFromHand = var->nguhanh ? captured
                               : !drop_loop() && is_promoted(to) ? make_piece(~color_of(captured), main_promotion_pawn_type(color_of(captured))) : ~captured;
