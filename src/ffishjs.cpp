...
public:
  ...
  std::string best_move(int movetime) {
    if (!Threads.size())
      return "";

    Search::LimitsType limits;
    limits.startTime = now();
    limits.movetime = movetime > 0 ? movetime : 1000;

    StateListPtr searchStates(new std::deque<StateInfo>(*states));
    Threads.start_thinking(pos, searchStates, limits, false);
    Threads.main()->wait_for_search_finished();

    Thread* bestThread = Threads.main()->bestThread ? Threads.main()->bestThread : Threads.main();
    if (   bestThread->rootMoves.empty()
        || bestThread->rootMoves[0].pv.empty()
        || bestThread->rootMoves[0].pv[0] == MOVE_NONE)
      return "";

    return UCI::move(pos, bestThread->rootMoves[0].pv[0]);
  }
...
private:
  ...
  void init(std::string uciVariant, std::string fen, bool is960) {
    ...
    std::string normalizedVariant = (uciVariant.size() == 0 || uciVariant == "Standard" || uciVariant == "standard")
      ? "chess"
      : uciVariant;
    v = get_variant(uciVariant);
    Options["UCI_Variant"] = normalizedVariant;
    UCI::init_variant(v);
    PSQT::init(v);
...
EMSCRIPTEN_BINDINGS(ffish_js) {
  class_<Board>("Board")
    ...
    .function("bestMove", select_overload<std::string()>(&Board::best_move))
    .function("bestMove", select_overload<std::string(int)>(&Board::best_move))
