...
namespace {
  ...
  Bitboard nguhanh_legal_destinations(const Position& pos, Color us, PieceType pt, Bitboard b) {
    ...
    Bitboard captures = 0;
    if (pt == KING)
        captures = pos.pieces(~us) & ~pos.pieces(~us, KING);
    else if (is_nguhanh_piece(pt))
        for (PieceSet victims = NGUHANH_PIECES; victims; )
        {
            PieceType victim = pop_lsb(victims);
            if (nguhanh_piece_can_capture(pt, victim))
                captures |= pos.pieces(~us, victim);
        }

    return (b & ~pos.pieces()) | (b & captures);
  }

  ExtMove* make_nguhanh_births(const Position& pos, ExtMove* moveList, Color us, Square from, Square to) {

    PieceType moved = type_of(pos.piece_on(from));
    if (   !pos.nguhanh()
        || !is_nguhanh_piece(moved)
        || !pos.count(us, KING)
        || !(nguhanh_steps_bb(to) & pos.pieces(us, KING)))
        return moveList;

    PieceType born = nguhanh_generated_piece(moved);
    if (!pos.can_drop(us, born))
        return moveList;

    Bitboard occupiedAfterMove = (pos.pieces() ^ from) | to;
    Bitboard birthSquares = nguhanh_steps_bb(to) & pos.board_bb() & ~occupiedAfterMove;

    while (birthSquares)
        *moveList++ = make_gating<NORMAL>(from, to, born, pop_lsb(birthSquares));
  ...
  ExtMove* make_move_and_gating(const Position& pos, ExtMove* moveList, Color us, Square from, Square to, PieceType pt = NO_PIECE_TYPE) {
    ...
    *moveList++ = make<T>(from, to, pt);
    ...
    if (T == NORMAL)
        moveList = make_nguhanh_births(pos, moveList, us, from, to);
  ...
  ExtMove* generate_moves(const Position& pos, ExtMove* moveList, PieceType Pt, Bitboard target) {
    ...
    while (bb)
    {
        ...
        Bitboard b = (  (attacks & pos.pieces())
                       | (quiets & ~pos.pieces()));
        b = nguhanh_legal_destinations(pos, Us, Pt, b);
  ...
  ExtMove* generate_all(const Position& pos, ExtMove* moveList) {
    ...
    if (pos.count<KING>(Us) && (!Checks || pos.blockers_for_king(~Us) & ksq))
    {
        ...
        b = nguhanh_legal_destinations(pos, Us, KING, b);
        while (b)
