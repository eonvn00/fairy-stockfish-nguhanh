...
public:
  ...
  bool nguhanh() const;
  bool nguhanh_extra_turn(Move m) const;
...
inline bool Position::nguhanh() const {
  assert(var != nullptr);
  return var->nguhanh;
}

inline bool Position::nguhanh_extra_turn(Move m) const {
  return nguhanh() && is_gating(m);
}
...
inline Bitboard Position::attacks_from(Color c, PieceType pt, Square s) const {
  if (var->nguhanh && (pt == KING || is_nguhanh_piece(pt)))
      return nguhanh_steps_bb(s) & board_bb(c, pt);
